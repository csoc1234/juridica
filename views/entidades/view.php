...
$representante = Dignatarios::find()->where(['and',['id_entidad' => $model->id_entidad],['representante_legal'=> 1],['estado'=>1]])->orderBy(['fin_periodo' => SORT_DESC])->limit(1)->one();
$this->title = $model->nombre_entidad;
$this->params['breadcrumbs'][] = ['label' => 'Entidades', 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;

$resoluciones_all = Resoluciones::find()->where(['id_entidad' => $model->id_entidad])->all();
$resoluciones = array();
for($i=0; $i<5; $i++){

  if(end($resoluciones_all)){
    array_push($resoluciones, array_pop($resoluciones_all));
  }
}

$certificados_all = Certificados::find()->where(['id_entidad' => $model->id_entidad])->all();
$certificados = array();
for($i=0; $i<5; $i++){
...
$tramite_devolucion_all = TramiteDevolucion::find()->where(['id_entidad' => $model->id_entidad])->all();
$tramite_devolucion = array();
for($i=0; $i<5; $i++){

  if(end($tramite_devolucion_all)){
    array_push($tramite_devolucion, array_pop($tramite_devolucion_all));
  }
}
