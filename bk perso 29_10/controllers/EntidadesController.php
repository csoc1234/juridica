...
class EntidadesController extends Controller
{
       ...
       if($radicado){
        ...
        // $interval1 = $fecha1->diff($fecha2); // la fecha de reconocimiento debe ser mayor o igual a la fecha de radicacion
          ...
          $b = $interval2->format('%R%a');
            if($model->fecha_reconocimiento == null){
                $model->fecha_reconocimiento = "9999-12-31";
              }
            if($model->fecha_gaceta == null){
              $model->fecha_gaceta = "9999-12-31";
            }
          if( $b >= 0){
     ...
     public function actionUpdate($id)
 {
   ...
   $radicado = Radicados::findOne($idRadicado);
   $model = $this->findModel($id);
   if((Yii::$app->user->identity->id_rol == 1 && $radicado) || (Yii::$app->user->identity->id_rol == 2 && $radicado) ){
     //$model = $this->findModel($id);
       ...
       if($model->file == null){
         ...
         foreach ($archivos as $key => $value) {
          ...
          if ($model->fecha_gaceta != $oldModel->fecha_gaceta){
            array_push($array_nombre_cambio,"CAMBIO DE FECHA GACETA ENTIDAD");
            array_push($array_campo_cambio,"fecha_gaceta");
          ...
          if ($model->periodo_entidad != $oldModel->periodo_entidad){
            array_push($array_nombre_cambio,"CAMBIO DEL PERIODO DE LA ENTIDAD");
            array_push($array_campo_cambio,"periodo_entidad");
        ...
        if ($model->load(Yii::$app->request->post()) &&  $model->save()) {
          if ($model->fecha_gaceta != $oldModel->fecha_gaceta){
            $this->UpdateHistory("CAMBIO DE FECHA GACETA ENTIDAD","fecha_gaceta","ENTIDADES", $model, $oldModel);
            $bandera2 = true;
          }
        }
